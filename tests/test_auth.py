"""Unit-Tests für auth.py (TokenBundle)."""
...
class TestTokenBundle:
    """Tests für TokenBundle Klasse."""

    def test_from_response(self):
        """Test TokenBundle.from_response() mit gültigen Daten."""
        payload = {
    ...
    def test_from_response_without_refresh_token(self):
        """Test TokenBundle.from_response() ohne refresh_token."""
        payload = {
            "access_token": "test_access_token",
            "expires_in": 3600,
            "scope": "test_scope",
    ...
    def test_save_and_from_file(self, valid_token_bundle: TokenBundle, temp_token_file: Path):
        """Test TokenBundle.save() und from_file()."""
        # Speichere TokenBundle
    ...
    def test_from_file_nonexistent(self, temp_dir: Path):
        """Test TokenBundle.from_file() mit nicht existierender Datei."""
    ...
    def test_is_expired_valid(self, valid_token_bundle: TokenBundle):
        """Test is_expired() mit gültigem Token."""
        assert not valid_token_bundle.is_expired()

    def test_is_expired_expired(self, expired_token_bundle: TokenBundle):
        """Test is_expired() mit abgelaufenem Token."""
        assert expired_token_bundle.is_expired()

    def test_from_file_invalid_json(self, temp_dir: Path):
        """Test TokenBundle.from_file() mit ungültigem JSON."""
        invalid_file = temp_dir / "invalid.json"
        invalid_file.write_text("invalid json")
        
        # from_file sollte None zurückgeben oder einen Fehler werfen
        # Aktuell wirft es einen Fehler beim JSON-Parsing
    ...
    def test_from_file_missing_fields(self, temp_dir: Path):
        """Test TokenBundle.from_file() mit fehlenden Feldern."""
        incomplete_file = temp_dir / "incomplete.json"
        incomplete_file.write_text('{"access_token": "test"}')
        
        # from_file sollte einen Fehler werfen, wenn expires_at fehlt
